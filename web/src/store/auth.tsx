...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(async (email: string, password: string): Promise<LoginResult> => {
    const { status, data } = await api.postWithStatus<{ user: User } | LoginPending>('/api/auth/login', { email, password })
  ...
  const value = useMemo(
    () => ({ user, needsSetup, login, completeLogin, refresh, setup, logout, clear }),
    [user, needsSetup, login, completeLogin, refresh, setup, logout, clear],
  )
